...
class PdfReportGenerator {
  ...
  private renderHeader(): void {
    ...
    this.doc.fontSize(7)
       ...
       .text(COMPANY.name, PAGE.marginLeft, y + 12);
    
    // Audited website in center
    this.doc.fontSize(8)
       .fillColor(COLORS.secondary)
       .text(this.data.websiteUrl, PAGE.marginLeft + 150, y + 5, { width: 200, align: "center" });
    
    // Date on the right
    this.doc.fontSize(8)
       .fillColor(COLORS.secondary)
       .text(formatDate(this.data.createdAt), PAGE.width - PAGE.marginRight - 100, y + 2, { width: 100, align: "right" });
    
    // Report ID
    this.doc.fontSize(7)
       .fillColor(COLORS.textLight)
       .text(`Отчёт #${this.data.auditId}`, PAGE.width - PAGE.marginRight - 100, y + 14, { width: 100, align: "right" });
    
    // Separator line
    this.doc.moveTo(PAGE.marginLeft, y + 26)
       .lineTo(PAGE.width - PAGE.marginRight, y + 26)
  ...
  private renderFooter(pageNum: number, totalPages: number): void {
    ...
    // Left: Page number and company info
    this.doc.fontSize(7)
       .fillColor(COLORS.textLight)
       .text(`Стр. ${pageNum}/${totalPages}`, PAGE.marginLeft, y + 6, { width: 50 });
    
    this.doc.fontSize(6)
       .fillColor(COLORS.textLight)
       .text(`ИНН ${COMPANY.inn}`, PAGE.marginLeft, y + 15, { width: 80 });
    
    // Center: Disclaimer and contacts
    this.doc.fontSize(6)
       .fillColor(COLORS.textLight)
       .text(
         ...
         PAGE.marginLeft + 100,
         y + 6,
         { width: PAGE.contentWidth - 200, align: "center" }
       );
    ...
    this.doc.fontSize(6)
       ...
       .text(
         ...
         PAGE.marginLeft + 80,
         y + 16,
         { width: PAGE.contentWidth - 160, align: "center" }
       );
