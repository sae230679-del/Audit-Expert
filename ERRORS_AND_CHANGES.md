...
- **Результат**: Комиссия **НИКОГДА** не начислялась через webhook YooKassa.
- **Исправление**: Заменить `getUserByReferralCode(user.referredBy)` на `storage.getUser(user.referredBy)`. Также добавлена привязка к referral записи и обновление статуса referral на 'paid'.
- **Статус**: ИСПРАВЛЕНО (2026-02-10)
...
- **Результат**: Реферер не видел своих приглашённых пользователей в кабинете.
- **Исправление**: Добавлен `await storage.createReferral({ referrerId, refereeId, status: 'registered', commissionEarned: 0 })` при регистрации.
- **Статус**: ИСПРАВЛЕНО (2026-02-10)
...
- **Результат**: В тестовом режиме реферальные комиссии не начислялись.
- **Исправление**: Экспортирована функция `processReferralCommission`, добавлен её вызов в ветку тестового режима.
- **Статус**: ИСПРАВЛЕНО (2026-02-10)
...
- **Исправление**: Добавлено извлечение JWT-токена из заголовка Authorization (необязательная авторизация) и передача `userId` в `createOrder`.
- **Статус**: ИСПРАВЛЕНО (2026-02-10)
...
- **Результат**: OAuth-пользователи не имели настроек уведомлений по умолчанию.
- **Исправление**: Добавлен `createOrUpdateSubscription` и `dispatchNotification('user_registered')` в VK и Yandex OAuth callback при создании нового пользователя.
- **Статус**: ИСПРАВЛЕНО (2026-02-10)
...
- **Исправление**: Реализован in-memory store для OAuth state с TTL 10 минут. State генерируется через `crypto.randomBytes(16)`, сохраняется при создании OAuth URL, проверяется и удаляется при callback. Автоматическая очистка просроченных записей.
- **Статус**: ИСПРАВЛЕНО (2026-02-10)
...
- **ИСПРАВЛЕНЫ ВСЕ КРИТИЧЕСКИЕ И ВАЖНЫЕ БАГИ:**
  - BUG-001: Исправлен поиск реферера (getUser вместо getUserByReferralCode)
  - BUG-002: Добавлено создание записи referral при регистрации
  - BUG-004: Добавлено начисление комиссии в тестовом режиме
  - BUG-006: Добавлена привязка userId к заказам при оплате
  - BUG-008: OAuth VK/Yandex: добавлено создание userSubscriptions и welcome notification
  - BUG-009: OAuth CSRF: реализована проверка state через in-memory store
  - Исправлены LSP-ошибки в storage.ts (documents.userId → clientUserId, типы id)
